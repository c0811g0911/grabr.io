...
import {Payment} from '../../components/_payment/Payment';
...
export class PaymentSettingsPage extends React.Component {
  static displayName = 'PaymentSettingsPage';
  ...
  componentDidMount() {
    window.scrollTo(0, 0);
    trackPageView(this.context, {path: `/settings/payment`});
  }
  ...
  render() {
    return (
      <Page>
        <Head>
          <title>
            {this.context.intl.formatMessage({id: 'pages.payment.document_title'})}
          </title>
        </Head>
        <Body>
        <div className="min-h-100 flex-col">
          ...
          <div className="flex-grow container-fluid w-100 m-md-b-3 m-t-2">
            <h1 className="text-center font-size-xl m-b-2">
              <FormattedMessage id="pages.payment.page_header"/>
            </h1>
            <div className="row">
...
// WEBPACK FOOTER //
// ./src/main/app/pages/_payment-settings/PaymentSettingsPage.js